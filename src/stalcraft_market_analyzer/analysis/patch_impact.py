...
PriceImpact = Literal["up", "down", "neutral"]
Severity = Literal["critical", "high", "medium", "low"]


class PatchItemImpact(BaseModel):
    item_name: str = Field(min_length=1)
    price_impact: PriceImpact = Field(default="neutral")


class PatchImpactStructured(BaseModel):
    severity: Severity = Field(default="medium")
    impact: PatchImpact = Field(default="NEUTRAL")
    items: list[PatchItemImpact] = Field(default_factory=list)
    ...
    def _derive_items_from_legacy_lists(self) -> "PatchImpactStructured":
        if self.items:
            return self
        derived: list[PatchItemImpact] = []
        for name in self.buffed_items:
            cleaned = str(name).strip()
            if cleaned:
                derived.append(PatchItemImpact(item_name=cleaned, price_impact="up"))
        for name in self.nerfed_items:
            cleaned = str(name).strip()
            if cleaned:
                derived.append(PatchItemImpact(item_name=cleaned, price_impact="down"))
        if not derived:
            return self
        return self.model_copy(update={"items": derived})
...
def analyze_patch_notes(*, patch_version: str, patch_text: str) -> dict[str, Any]:
    ...
    if not cfg.enabled:
        ...
        return finalize_patch_result(patch_version=patch_version, source="stub", structured=stub)
    ...
    if not api_key:
        ...
        return finalize_patch_result(patch_version=patch_version, source="stub_missing_key", structured=stub)

    structured = _openai_patch_impact(patch_version=patch_version, patch_text=patch_text, model=cfg.model)
    return finalize_patch_result(
        patch_version=patch_version,
        source=f"openai:{cfg.model}",
...
def finalize_patch_result(
    *,
    patch_version: str,
    source: str,
    structured: PatchImpactStructured,
) -> dict[str, Any]:
    payload = structured.model_dump()
    buffed, nerfed = derive_legacy_item_lists(payload.get("items") or [])
    if not payload.get("buffed_items"):
        payload["buffed_items"] = buffed
    if not payload.get("nerfed_items"):
        payload["nerfed_items"] = nerfed
    return {"patch_version": patch_version, "source": source, **payload}


def derive_legacy_item_lists(
    ...
    for row in items:
        ...
        if not name:
            continue
        impact = str(row.get("price_impact", "neutral")).strip().lower()
        if impact == "up":
            buffed.append(name)
        elif impact == "down":
            nerfed.append(name)
...
def _openai_patch_impact(*, patch_version: str, patch_text: str, model: str) -> PatchImpactStructured:
    ...
    system = (
        ...
        "items (array of objects with item_name string and price_impact one of up|down|neutral), "
        "confidence (number 0..1), "
        "notes (short string). "
        "Use price_impact=up when the patch likely raises demand/price, down when it lowers, neutral otherwise. "
        "Prefer filling items[]; buffed_items/nerfed_items are optional legacy fields."
    )

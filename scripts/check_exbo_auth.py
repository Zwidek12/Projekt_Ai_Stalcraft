...
def main() -> int:
    ...
    if not headers:
        ...
        print("Demo (README): EXBO_API_BASE_URL=https://dapi.stalcraft.net EXBO_CLIENT_ID=1 EXBO_CLIENT_SECRET=E98cm6J9NNjTQopph0c2eIXNKafg4R1Cjz0TZh2D")
        return 1
    ...
    if history_summary["with_qlt"] == 0 and lots_summary["with_qlt"] == 0:
        print("Auth works (HTTP 200), but this probe did not return additional.qlt for the selected item.")
        if "dapi.stalcraft.net" in base_url:
            print(
                "Note: demo API (dapi) often returns empty additional={}. "
                "For real qlt use production EXBO_API_BASE_URL=https://eapi.stalcraft.net "
                "with your own EXBO_CLIENT_ID + EXBO_CLIENT_SECRET."
            )
    else:
